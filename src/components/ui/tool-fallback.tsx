...
function ToolFallbackTrigger({
  ...
  const label = isCancelled ? 'Công cụ đã hủy' : 'Công cụ đã dùng';
...
function ToolFallbackArgs({
  ...
  return (
    <div
      ...
      {...props}
    >
      <pre className="aui-tool-fallback-args-value bg-muted/50 text-muted-foreground rounded-md p-2.5 text-xs whitespace-pre-wrap font-mono">
        {argsText}
      </pre>
    </div>
  );
...
function ToolFallbackResult({
  ...
  return (
    ...
    >
      ...
      <pre className="aui-tool-fallback-result-content bg-muted/50 text-muted-foreground mt-1 rounded-md p-2.5 text-xs whitespace-pre-wrap font-mono">
...
function ToolFallbackError({
  ...
  const headerText = isCancelled ? 'Lý do hủy:' : 'Lỗi:';
...
const ToolFallbackImpl: ToolCallMessagePartComponent = ({
  ...
  return (
    <ToolFallbackRoot open={open} onOpenChange={setOpen} className="bg-white dark:bg-slate-900 border border-border/80 rounded-xl shadow-sm p-3 mt-2">
